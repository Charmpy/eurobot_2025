#include <cv_bridge/cv_bridge.hpp> //changed
...
#define ser (*pser)
 ...
 public:
  SipeedTOF_MSA010_Publisher() : Node("sipeed_tof_ms_a010_right") {
    std::string s;
    ...
    publisher_depth =
        this->create_publisher<sensor_msgs::msg::Image>("depth_right", 10);
    publisher_pointcloud =
        this->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_right", 10);
 ...
 private:
  ...
  _more:
    ...
    RCLCPP_INFO(this->get_logger(), "Publishing: depth_right:%s",
                sstream.str().c_str());
